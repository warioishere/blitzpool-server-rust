...
const CLIENT_VIEW_ADDR: &str = "bc1qcpmga8eqxzwz4nmnew6v6xkapggs3sqzpu4fp0";
...
async fn client_address_reports_the_custom_extranonce_per_worker() {
    ...
    let cleanup_rows = |pool: PgPool| async move {
        let _ = sqlx::query("DELETE FROM pplns_custom_extranonce WHERE address = $1")
            .bind(CLIENT_VIEW_ADDR)
            .execute(&pool)
            .await;
        let _ = sqlx::query("DELETE FROM client_entity WHERE address = $1")
            .bind(CLIENT_VIEW_ADDR)
            .execute(&pool)
            .await;
    };
    ...
    // Two live workers on the address; only `rig1` gets an override.
    for (worker, session) in [("rig1", "aaaa1111"), ("rig2", "bbbb2222")] {
        sqlx::query(
            r#"INSERT INTO client_entity
                 (address, "clientName", "sessionId", "startTime")
               VALUES ($1, $2, $3, 0)"#,
        )
        .bind(CLIENT_VIEW_ADDR)
        .bind(worker)
    ...
    seed_token(&pool, CLIENT_VIEW_ADDR, "tok-client-view").await;
    let body = format!(
        r#"{{"address":"{CLIENT_VIEW_ADDR}","workers":[{{"worker":"rig1","extranonce":"0c0de001"}}]}}"#
    );
    let (st, js) = post_json_auth(
        build_router(minimal_state(pool.clone())),
        "/api/address/extranonce/set",
        body,
        Some("tok-client-view"),
    )
    .await;
    ...
    assert_eq!(st, StatusCode::OK, "client view must succeed; got {js}");
    let workers = js["workers"].as_array().expect("workers array");
    let entry = |name: &str| {
        workers
            .iter()
            .find(|w| w["name"] == name)
            .unwrap_or_else(|| panic!("{name} missing from {js}"))
            .clone()
    };
    assert_eq!(
        entry("rig1")["extranonce"],
        "0c0de001",
        "the configured worker must report its stored prefix; got {js}"
    );
    assert!(
        entry("rig2")["extranonce"].is_null(),
        "a worker without an override must report null, not a borrowed prefix; got {js}"
    );
}
