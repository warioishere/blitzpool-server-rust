...
// ── sum_active_pool_hashrate: staleness decay ────────────────────────

#[tokio::test]
async fn pool_hashrate_decays_stale_sessions() {
    let Some(pool) = connect_or_skip().await else {
        return;
    };
    let mut tx = pool.begin().await.expect("begin tx");

    // Far-future reference clock: every pre-existing / concurrently-inserted row
    // has a real `updatedAt` (~now) and is therefore ancient relative to this
    // clock (staleness >> window) → decays to 0. So only the rows we insert here
    // with a near-`now` `updatedAt` contribute, making the assertion exact
    // without depending on what else is in the table.
    let now = 4_000_000_000_000i64;
    let w = 120_000i64; // 2 min

    // fresh → factor 1.0 (100), half-window stale → 0.5 (50), past window → 0.
    // sessionId is varchar(8), so keep these short.
    for (sid, updated_at, hr) in [
        ("dcyFresh", now, 100.0f64),
        ("dcyHalf", now - 60_000, 100.0),
        ("dcyGone", now - 200_000, 100.0),
    ] {
        sqlx::query(
            r#"INSERT INTO client_entity
                 (address, "clientName", "sessionId", "startTime", "hashRate", "updatedAt")
               VALUES ($1, $2, $3, $4, $5, $6)"#,
        )
        .bind("bp_decay_test_addr")
        .bind("wkr")
        .bind(sid)
    ...
    let total = bp_db::sum_active_pool_hashrate(&mut *tx, now, w)
        .await
        .expect("sum_active_pool_hashrate");
    assert!(
        (total - 150.0).abs() < 1e-6,
        "expected 100 + 50 + 0 = 150 H/s after decay, got {total}"
    );

    // tx dropped without commit → rolls back, no cleanup needed.
}
