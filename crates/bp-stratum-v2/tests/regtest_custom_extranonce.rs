...
async fn sv2_custom_extranonce_applies_at_open_and_live_and_leaves_others_untouched() {
    ...
    eprintln!(
        "[custom-en] open custom: open={:02x?} set={:02x?}",
        c_open.open_prefix, c_open.set_prefix
    );
    eprintln!(
    ...
    assert_eq!(
        ...
        "override miner's OpenExtendedMiningChannelSuccess must carry PREFIX_1"
    );
    assert!(
        c_open.set_prefix.is_none(),
        "no SetExtranoncePrefix is needed at open any more; got {:02x?}",
        c_open.set_prefix
    );
    // Negative control on the same run: a normal miner keeps the allocated
    // prefix, so the assertion above cannot pass by the pool sending PREFIX_1
    // to everyone.
    assert_ne!(
        n_open.open_prefix.as_deref(),
        Some(PREFIX_1.as_slice()),
        "normal miner must NOT receive the customer prefix"
    );
    assert!(
...
struct OpenObs {
    /// The prefix carried by `OpenExtendedMiningChannelSuccess` — where every
    /// SV2 client, including firmware that never implements
